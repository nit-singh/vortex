...
class OnlineRiskScorer:
    def __init__(
        ...
        # Store offline centroids for consistent distance calculations
        self.offline_centroids = offline_centroids
        
        # Initialize River KMeans centers from offline centroids if available
        if offline_centroids is not None:
            self._initialize_centers_from_offline(offline_centroids)

    def _initialize_centers_from_offline(self, offline_centroids):
        """Initialize River KMeans centers to match offline sklearn centroids."""
        centers = _ensure_centers(self.kmeans)
        for cluster_id, centroid_array in enumerate(offline_centroids):
            centers[cluster_id] = {f"f{i}": float(val) for i, val in enumerate(centroid_array)}
        self.kmeans.centers = centers
    ...
    def _center_array(self, cluster_id):
        """Get centroid as numpy array, prioritizing offline centroids if available."""
        if self.offline_centroids is not None and cluster_id < len(self.offline_centroids):
            return self.offline_centroids[cluster_id]
        
        # Fallback to River KMeans centers
        centers = _ensure_centers(self.kmeans)
        center_dict = centers.get(cluster_id, {})
        return np.array([center_dict.get(f"f{i}", 0.0) for i in range(self.latent_dim)])

    def get_online_centroid(self, cluster_id):
        """Get the current online (River KMeans) centroid for a cluster."""
        centers = _ensure_centers(self.kmeans)
        center_dict = centers.get(cluster_id, {})
        return np.array([center_dict.get(f"f{i}", 0.0) for i in range(self.latent_dim)])

    def get_centroid_drift(self):
        """Calculate drift between offline and online centroids.
        
        Returns dict with drift metrics per cluster and overall summary.
        """
        if self.offline_centroids is None:
            return {"error": "No offline centroids available for comparison"}
        
        drift_metrics = {
            "per_cluster": {},
            "total_drift": 0.0,
            "max_drift": 0.0,
            "max_drift_cluster": None,
        ...
        for cluster_id, offline_centroid in enumerate(self.offline_centroids):
            online_centroid = self.get_online_centroid(cluster_id)
            drift_distance = float(np.linalg.norm(online_centroid - offline_centroid))
            
            drift_metrics["per_cluster"][cluster_id] = {
                "drift_distance": drift_distance,
                "offline_centroid": offline_centroid.tolist(),
                "online_centroid": online_centroid.tolist(),
            }
            
            total_drift += drift_distance
            if drift_distance > max_drift:
                max_drift = drift_distance
                max_cluster = cluster_id
        
        drift_metrics["total_drift"] = total_drift
        drift_metrics["avg_drift"] = total_drift / len(self.offline_centroids) if self.offline_centroids else 0.0
        drift_metrics["max_drift"] = max_drift
        drift_metrics["max_drift_cluster"] = max_cluster
    ...
    def predict_and_update(self, sample_dict, update=True):
        ...
        # Use offline centroids for prediction if available
        if self.offline_centroids is not None:
            # Direct distance calculation to offline centroids
            distances = [np.linalg.norm(latent_vec - centroid) for centroid in self.offline_centroids]
            cluster_id = int(np.argmin(distances))
            dist = distances[cluster_id]
        else:
            # Fallback to River KMeans prediction
        ...
        # Update River KMeans centroids if explicitly requested
        # Note: When using offline_centroids, prediction uses fixed centroids,
        # but we can still update River KMeans for drift tracking/comparison
        if update:
            latent_dict = self._latent_to_dict(latent_vec)
            self.kmeans = kmeans_learn_manual(self.kmeans, latent_dict, lr=0.2)
            self.update_cnt += 1
        ...
        return {
            ...
            "centroids_updated": update,
            "using_offline_centroids": self.offline_centroids is not None,
        }

    @staticmethod
    def load(directory, autoencoder_class, input_dim, latent_dim, use_offline_centroids=True):
        ...
        # Load offline centroids if available and requested
        offline_centroids = None
        if use_offline_centroids and "centroids" in cluster_context:
            offline_centroids = [np.array(c) for c in cluster_context["centroids"]]
        ...
        return OnlineRiskScorer(
            ...
            offline_centroids=offline_centroids,
        )


def load_deployment_pipeline(artifact_dir: str, use_offline_centroids: bool = True) -> Tuple[OnlineRiskScorer, Any, Dict[str, Any]]:
...
def format_report(offline_result, online_result):
    ...
    # Show discrepancy
    score_diff = abs(offline_result['risk_score'] - online_result['risk_score'])
    cluster_match = offline_result['cluster_id'] == online_result['cluster_id']
    print(f"\n=== Discrepancy Analysis ===")
    print(f"Cluster Match       : {'✓ YES' if cluster_match else '✗ NO'}")
    print(f"Risk Score Diff     : {score_diff:.2f}")
    if score_diff < 1.0:
        print(f"Status              : ✓ Excellent alignment (< 1.0 difference)")
    elif score_diff < 5.0:
        print(f"Status              : ✓ Good alignment (< 5.0 difference)")
    else:
        print(f"Status              : ⚠ Large discrepancy detected")
...
def build_arg_parser():
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="Disable use of offline centroids in online scorer (may cause discrepancy).",
    )
    return parser
...
def main():
    ...
    scorer, offline_kmeans, cluster_context = load_deployment_pipeline(
        artifact_dir, 
        use_offline_centroids=use_offline_centroids
    )
    
    if use_offline_centroids:
        print("✓ Online scorer initialized with offline centroids for consistency")
    else:
        print("⚠ Online scorer using River KMeans centroids (may diverge from offline)")
