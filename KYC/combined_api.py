...
from kyc_observability import increment, track_latency
...
async def parser_paddleocr(img):
    """PaddleOCR Parser - extracts text from image"""
    ...
    # Call the async method directly - returns list of (text, metadata) tuples
    results = await ocr_parser.__wrapped__(img_bytes)
...
async def parser_docling(img):
    """Docling Hybrid Parser - extracts text from image using Docling + Unstructured"""

    # ====================================================================
    # UNIVERSAL HYBRID PARSER (Docling + Unstructured)
    # ====================================================================

    class HybridDocumentParser(pw.UDF):
        def __init__(
            self,
            docling_parser=None,
            ...
            if docling_parser is None:
                try:
                    ...
                    # Handle case where interpreter is shutting down or models can't be downloaded
                    if "cannot schedule new futures after interpreter shutdown" in str(e):
                        # Fallback: return None and use unstructured only
                        docling_parser = None
                    else:
                        raise
        ...
        def _extract_with_unstructured(self, contents: bytes) -> str:
            try:
                ...
                # Handle None values and ensure all elements convert to strings properly
                text_parts = []
                for element in elements:
                    ...
                    try:
                        ...
                        # Check if __str__ returned None (shouldn't happen but handle it)
                        if element_str is None:
                            continue
                        text_parts.append(element_str)
        ...
        async def _parse_async(self, contents: bytes) -> Dict[str, Any]:
            # Try to use docling parser, fallback to unstructured if unavailable
            docling_result = []
            docling_text = ""
            
            if self.docling_parser is not None:
                try:
                    docling_result = await self.docling_parser.parse(contents)
                    docling_text = "\n".join([chunk for chunk, _ in docling_result])
                except (RuntimeError, Exception) as e:
                    # Handle interpreter shutdown or other errors
                    if "cannot schedule new futures after interpreter shutdown" in str(e):
                        # Fallback to unstructured only
                        docling_result = []
                        docling_text = ""
                    else:
                        raise
            ...
            # If docling failed, use unstructured text as primary
            if not docling_result:
                return {
                    'parsed_chunks': [(unstructured_text, {})],
        ...
        def __wrapped__(self, contents: bytes, **kwargs) -> Dict[str, Any]:
            loop = asyncio.get_event_loop()
            return loop.run_until_complete(self._parse_async(contents))

    buf = BytesIO()
    img.save(buf, format='PNG')
    img_bytes = buf.getvalue()

    # Call the async wrapped method directly
    parser = HybridDocumentParser()
    results = await parser._parse_async(img_bytes)
...
class PANCardExtractor:
    ...
    def extract_father_name(self, text: str, name: Optional[str] = None) -> Optional[str]:
    ...
    def extract_all(self, ocr_output: Any) -> Dict[str, Any]:
    ...
    def paddleocr_to_text(self, ocr_result: List[Any]) -> str:
        text_lines = []
...
async def parser_(image: Image.Image, doc_type: str) -> Dict[str, Any]:
    """
    Global parser function that routes to appropriate parser and extractor
    ...
    if doc_type == 'pan':
        raw_text = await parser_paddleocr(image)
        result = pan_extract(raw_text)
        return result
    
    elif doc_type == 'adhaar':
        raw_text = await parser_docling(image)
...
async def process_uploaded_file(file: UploadFile) -> Image.Image:
    """Process uploaded file - convert PDF to image if needed, otherwise return image"""
...
class VideoVerificationService:
    ...
    def __init__(self):
        ...
        self.head_motion_required = 0.10
...
@app.post("/api/v1/kyc/verify-complete")
async def verify_kyc_complete(
    pan_document: UploadFile = File(..., description="PAN card image or PDF"),
    aadhaar_document: UploadFile = File(..., description="Aadhaar card image or PDF"),
    itr_document: UploadFile = File(..., description="ITR document image or PDF"),
    selfie_video: UploadFile = File(..., description="Selfie video for verification"),
    questionnaire: str = Form(..., description="Questionnaire responses as JSON"),
    additional_details: str = Form(..., description="Additional personal details as JSON")
):
    """
    Complete integrated KYC verification endpoint
    
    This endpoint:
    1. Accepts PAN, Aadhaar, ITR documents (as images or PDFs) and selfie video
    2. Converts PDFs to images if needed
    3. Parses all documents using the global parser
    4. Cross-verifies the extracted information
    5. Returns master verification JSON and ML model input JSON
    """
    
    labels = {"endpoint": "verify_complete"}

    with track_latency("api.verify_complete", **labels):
        try:
            # Parse JSON inputs
            questionnaire_input = QuestionnaireInput(**json.loads(questionnaire))
            additional_input = AdditionalDetails(**json.loads(additional_details))

            # Step 1: Process uploaded files (convert PDF to image if needed)
            pan_image = await process_uploaded_file(pan_document)
            aadhaar_image = await process_uploaded_file(aadhaar_document)
            itr_image = await process_uploaded_file(itr_document)

            # Step 2/3/4: Run PAN/Aadhaar/ITR parsing + video verification concurrently
            # We already converted uploaded files to PIL images above. To allow the
            # blocking video pipeline (which uses OpenCV/torch) to run without
            # blocking the event loop, we run it in the thread executor. The
            # parsers are async coroutines and are scheduled as asyncio tasks.
            pan_input = PANCardInput()
            aadhaar_input = AadhaarCardInput()
            itr_input = ITRDocumentInput()
            pan_data_dict: Dict[str, Any] = {}
            aadhaar_data_dict: Dict[str, Any] = {}
            itr_data_dict: Dict[str, Any] = {}
            ...
            video_verification_result: Dict[str, Any] = video_verifier.error_result("verification_not_started")

            temp_dir = tempfile.mkdtemp()
            try:
                pan_temp_path = os.path.join(temp_dir, "pan_document.png")
                aadhaar_temp_path = os.path.join(temp_dir, "aadhaar_document.png")
                itr_temp_path = os.path.join(temp_dir, "itr_document.png")
                selfie_suffix = os.path.splitext(selfie_video.filename or "selfie_video.mp4")[1] or ".mp4"
                video_temp_path = os.path.join(temp_dir, f"selfie_video{selfie_suffix}")

                # Persist images for the video pipeline (OpenCV expects file paths)
                pan_image.save(pan_temp_path, format='PNG')
                aadhaar_image.save(aadhaar_temp_path, format='PNG')
                itr_image.save(itr_temp_path, format='PNG')

                # Persist selfie video bytes
                await selfie_video.seek(0)
                with open(video_temp_path, "wb") as f:
                    # Use the UploadFile.file file-like to copy contents
                    shutil.copyfileobj(selfie_video.file, f)

                # Schedule async parser coroutines
                loop = asyncio.get_running_loop()
                parse_pan_task = asyncio.create_task(parser_(pan_image, 'pan'))
                parse_aadhaar_task = asyncio.create_task(parser_(aadhaar_image, 'adhaar'))
                parse_itr_task = asyncio.create_task(parser_(itr_image, 'itr'))

                # Run the blocking video verification in executor so it can use
                # native libraries / multiple cores without blocking the loop.
                video_task = loop.run_in_executor(
                    None, video_verifier.run_pipeline, aadhaar_temp_path, pan_temp_path, video_temp_path
                )

                # Wait for all tasks to complete concurrently
                pan_data_dict, aadhaar_data_dict, itr_data_dict, video_verification_result = await asyncio.gather(
                    parse_pan_task, parse_aadhaar_task, parse_itr_task, video_task
                )

                # Build pydantic objects from parsed dicts
                ...
                # Cross-verify documents (uses the parsed Pydantic inputs)
                document_verification = cross_verify_documents(pan_input, aadhaar_input, itr_input)
                increment(
                    "doc.mismatch_total",
                    value=len(document_verification.get("mismatches", [])),
                )
                increment(
                    "doc.warnings_total",
                    value=len(document_verification.get("warnings", [])),
                )
                increment(
                    "video.verifications",
                    value=1,
                    decision=video_verification_result.get("final_decision"),
                )
                ...
                video_verification_result = video_verifier.error_result(str(exc))
                pan_input = PANCardInput()
                aadhaar_input = AadhaarCardInput()
                itr_input = ITRDocumentInput()
                pan_data_dict = {}
            ...
            # Step 5: Resolve consolidated values from all sources
            ...
            # Resolve Gender
            gender_resolved = resolve_field_value("Gender", aadhaar_input.gender.value if aadhaar_input.gender else None)

            # Resolve Address
            address_resolved = resolve_field_value("Address", additional_input.address)

            # Resolve Financial details
            ...
            temp_master_json_id = str(uuid.uuid4())
            ...
            increment(
                ...
                severity=alert_plan.severity.value,
            )

            # Build master verification JSON with detailed status
            master_json = {
                "verification_status": {
                    "document_verification": document_verification["is_verified"],
                    "video_verification": video_verification_result["final_decision"] == "accept",
                    "overall_status": (
                        document_verification["is_verified"] and
                        video_verification_result["final_decision"] == "accept"
                    ),
                    "summary": {
            ...
            # Build ML model input JSON (using resolved values, None for missing)
            ml_input_json = {
                "age": age,
            ...
            master_json_id = register_master_json(master_json, temp_master_json_id)
            ...
            return JSONResponse(content={
                ...
                "alert_signal": alert_signal.to_dict(),
                "alert_plan": alert_plan.to_dict(),
            })
...
async def root():
    ...
    return {
        "message": "Integrated KYC Verification API",
        "version": "2.0.0",
        "endpoints": {
            "/api/v1/kyc/verify-complete": "POST - Complete integrated KYC verification (recommended)",
            "/parse": "POST - Parse individual document (legacy)",
            "/health": "GET - Health check"
        },
        "supported_documents": ["pan", "adhaar", "itr"]
    }
...
async def health_check():
    ...
    return {"status": "healthy", "service": "Integrated KYC Verification API"}
...
async def parse_document(
    ...
    """
    Parse document and extract information (legacy endpoint)
    
    Args:
        file: Image file (PNG, JPG, JPEG, PDF)
        doc_type: Type of document ('pan', 'adhaar', 'itr')
    ...
    try:
        ...
        # Parse document
        result = await parser_(image, doc_type)
