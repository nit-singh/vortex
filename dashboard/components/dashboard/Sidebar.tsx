...
export default function Sidebar({ userName = "Daksh Mewati", userType = "Premium" }: SidebarProps) {
  const pathname = usePathname();
  const router = useRouter();

  const navItems = [
    { icon: LayoutDashboard, label: "Dashboard", path: "/consumer/dashboard" },
    { icon: Wallet, label: "Portfolio", path: "/consumer/portfolio" },
    { icon: FileText, label: "Reports", path: "/consumer/reports" },
      ...
      <div className="p-4 border-t border-[#1a1a1a]">
        <div className="flex items-center gap-3 mb-3">
          ...
          <div className="flex-1">
            ...
            <p className="text-[#666666] text-xs">{userType}</p>
          </div>
        </div>
        <button
